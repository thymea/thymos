...
// SSFN
pub export var ssfn_src: *g.c.ssfn_font_t = undefined;
...
pub fn getCursorX() c_int {
    return ssfn_dst.x;
}
pub fn getCursorY() c_int {
    return ssfn_dst.y;
}
...
pub fn init(backgroundColor: u32, foregroundColor: u32) void {
    ...
    if (fbRequest.response) |response| {
        ...
        ssfn_src = @ptrCast(@constCast(&FONT[0]));
        ssfn_dst.ptr = @ptrCast(fb.address);
        ssfn_dst.w = @intCast(fb.width);
        ssfn_dst.h = @intCast(fb.height);
        ssfn_dst.p = @intCast(fb.pitch);
        ssfn_dst.x = 0;
        ssfn_dst.y = 0;
        ssfn_dst.bg = backgroundColor;
        ssfn_dst.fg = foregroundColor;
    } else @panic("No framebuffer available");
}

// Clear the screen and reset cursor position
pub fn resetScreen() void {
    drawFilledRect(0, 0, @intCast(fb.width), @intCast(fb.height), ssfn_dst.bg);
    ssfn_dst.x = 0;
    ssfn_dst.y = 0;
}
...
export fn _putchar(char: u8) void {
    ...
    switch (char) {
        ...
        '\n' => {
            ssfn_dst.x = 0;
            if ((ssfn_dst.y + GLYPH_HEIGHT) > fb.height) {
                resetScreen();
            } else ssfn_dst.y += GLYPH_HEIGHT;
        },

        // Tab
        '\t' => {
            if ((ssfn_dst.x + (GLYPH_WIDTH * TAB_SPACES)) > fb.width) {
                resetScreen();
            } else {
                for (0..4) |_|
                    _putchar(' ');
        ...
        0x08 => {
            ...
            if (ssfn_dst.x >= GLYPH_WIDTH) {
                ssfn_dst.x -= GLYPH_WIDTH;
            } else if (ssfn_dst.y >= GLYPH_HEIGHT) {
                ssfn_dst.y -= GLYPH_HEIGHT;
                ssfn_dst.x = @intCast(fb.width - GLYPH_WIDTH);
            } else return;

            // Draw over the previous character to "erase" it
            drawFilledRect(
                @intCast(ssfn_dst.x),
                @intCast(ssfn_dst.y),
                GLYPH_WIDTH,
